...
export default function DisplayQuestion  ()  {
   ...
   const obj=  useParams<{ questionId:string }>();
   const { data: question, isLoading, isError } = useQuery<question>(
    {queryKey: [ 'questions', obj?.questionId  ? +obj.questionId : -1] ,
    queryFn: async () => {
       const { data } = await axiosInstance.get(`/Question/${obj?.questionId}`);
       return data;
     },
...
const [unsavedVideo,setUnsavedVideo] = useState(false)
  ...
  useEffect(()=> {
    if(question) {
    setNextQuestionId(question?.nextQuestionId ? question.nextQuestionId : -1)
    setPreviousQuestionId(question?.previousQuestionId ? question.previousQuestionId : -1 )
    }
...
{previousQuestionId !== -1 && <Button className='self-end ml-3' onClick={()=> {changeQuestion(-1)}}>Go Back</Button>}
{nextQuestionId !== -1 && <Button className='self-start mr-10' onClick={()=> {changeQuestion(1)}}>Next Question</Button>}

</div>
<div className='mb-0 mt-3  flex justify-center items-c space-x-4 w-screen'>

{question?.interviewId && question.interviewId !== -1 && <Button className='' onClick={()=> {goToInterview()}}>View Interview Information</Button> }
</div>


        {question && 
       <> <InterviewRecord secondsPerAnswer={question.secondsPerAnswer} setUnsavedVideo={setUnsavedVideo} question={question} disabled={false}></InterviewRecord>
