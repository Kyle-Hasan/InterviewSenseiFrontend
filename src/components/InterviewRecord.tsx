...
export default function InterviewRecord({question,setUnsavedVideo,secondsPerAnswer}:InterviewRecordProps) {
  ...
  const sendForReview = async(question:question)=> {
    ...
    const response = await axiosInstance.post("/Response/rateAnswer",formData,{
      headers: {
  ...
  <div className='flex justify-center items-start space-x-10 w-full h-2/3 xl:h-[80%] lg:h-[78%]'>
    ...
    <div className='flex flex-col items-center border-2 border-black p-5 xl:w-2/5 w-1/3  h-full'>
      ...
      <VideoRecord secondsPerAnswer={secondsPerAnswer} question={question.body} setUnsavedVideo={setUnsavedVideo} videoLink={selectedResponseIndex >= 0 && responses && responses.length >= 0 && selectedResponseIndex < responses.length ? responses[selectedResponseIndex].videoLink : ""} responseLoading={loadingResponse} setBlob={setBlob} setRecording={setRecording} recording={recording} sendForReview={()=> {addResponse.mutate()}} />
      
    </div>
    {responses  && responses.length > 0 ?
    <div className='flex flex-col'>
    <p className='text-center mb-1'>Responses </p>
         <Select value={selectedResponseIndex.toString()} onValueChange={(e)=> {handleSelectChange(e)}} >
      ...
      <SelectContent>
        ...
        
        {
          responses?.map((x,index)=> {
           return  <SelectItem key={index} value={index.toString()}>Response {index+1}</SelectItem>
          })
        }
        </SelectGroup>
      </SelectContent>
    </Select> </div> : <p>No responses</p>
}
